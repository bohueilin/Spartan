...
import com.spartan.domain.model.Confidence
...
class InsightEngine(
    ...
    private fun vitaminDInsight(assessment: MetricAssessment): InsightCard = InsightCard(
        ...
        explanation = when {
            assessment.clinicalStatus == ClinicalStatus.BELOW_RANGE ->
                "This value is below basic adequacy in many references. One value is not a diagnosis."
            ...
            else -> "This value is above basic adequacy in many references."
        },
    ...
    private fun rhrInsight(assessment: MetricAssessment): InsightCard = InsightCard(
        ...
        explanation = when {
            assessment.clinicalStatus == ClinicalStatus.ABOVE_RANGE ->
                "This value is above the common clinical reference range. One value is not a diagnosis."
            assessment.clinicalStatus == ClinicalStatus.BELOW_RANGE ->
                "This value is below the common clinical reference range. Context and symptoms matter."
            ...
            else -> "This is within the normal range for many adults."
        },
    ...
    private fun bpInsight(assessment: MetricAssessment): InsightCard = InsightCard(
        ...
        explanation = when (assessment.clinicalStatus) {
            ClinicalStatus.NORMAL -> "Your current blood pressure entry is in the normal range."
            ClinicalStatus.ABOVE_RANGE -> "This blood pressure value is above the normal clinical range. Confirm with repeat seated readings."
            ClinicalStatus.BELOW_RANGE -> "This blood pressure value is below the common clinical range. Symptoms and repeat readings matter."
            ClinicalStatus.PENDING -> "This blood pressure entry is pending."
            ClinicalStatus.UNKNOWN -> "No standard clinical classification is shown for this blood pressure entry."
        },
